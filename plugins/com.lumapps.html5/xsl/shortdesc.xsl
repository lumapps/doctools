...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template
        match="*[contains(@class, ' task/taskbody ')][not(count(ancestor::*[contains(@class, ' topic/topic ')]) > 1)]"
                ...
                <xsl:with-param name="default-output-class">
                    <xsl:choose>
                        <xsl:when test="count(ancestor::*[contains(@class, ' topic/topic ')]) > 1">
                            <!-- when a shortdesc is in a chunked topic it has more than one topic above it
                            in this case it should not have the docWidget-shortdesc class -->
                            <xsl:value-of select="'shortdesc'"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <!-- when a shortdesc is in a chunked topic it should have the 
                            docWidget-shortdesc class -->
                            <xsl:value-of select="'docWidget docWidget-shortdesc shortdesc'"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:with-param>
            </xsl:call-template>
